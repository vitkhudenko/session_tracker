...
class SessionTrackerStorage(appContext: Application) : ISessionTrackerStorage<Session, Session.State> {
    ...
    companion object {
        ...
        private fun sessionRecordToJson(sessionRecord: SessionRecord<Session, Session.State>): JSONObject {
            return JSONObject(
                mapOf(
                    KEY_SESSION_ID to sessionRecord.session.sessionId,
                    KEY_SESSION_STATE to sessionRecord.state.ordinal
                )
            )
        }

        private fun jsonToSessionRecord(json: JSONObject): SessionRecord<Session, Session.State> {
            return SessionRecord(
                Session(json.getString(KEY_SESSION_ID)),
                Session.State.values()[json.getInt(KEY_SESSION_STATE)]
            )
        }
    }

    override fun readAllSessionRecords(): List<SessionRecord<Session, Session.State>> {
        val sessionRecords = mutableListOf<SessionRecord<Session, Session.State>>()
    ...
    override fun createSessionRecord(sessionRecord: SessionRecord<Session, Session.State>) {
        saveSessionRecords(
            readAllSessionRecords() + sessionRecord
        )
    }

    override fun updateSessionRecord(sessionRecord: SessionRecord<Session, Session.State>) {
        saveSessionRecords(
            readAllSessionRecords().map {
                if (it.session.sessionId == sessionRecord.session.sessionId) {
                    sessionRecord
                } else {
                    it
                }
    ...
    override fun deleteSessionRecord(sessionId: String) {
        saveSessionRecords(
            readAllSessionRecords().filter { it.session.sessionId != sessionId }
        )
    }
    ...
    private fun saveSessionRecords(sessionRecords: List<SessionRecord<Session, Session.State>>) {
        prefs.edit()
            .putString(
                KEY_SESSION_RECORDS,
                JSONArray(
